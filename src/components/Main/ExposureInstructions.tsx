    ...
    <FadeInView style={{ flex: 1 }}>
      <ContentContainer style={!IS_SMALL_SCREEN ? styles.container : {}} scrollEnabled showsVerticalScrollIndicator={false}>
        <View style={{ alignItems: 'center', paddingHorizontal: 30 }}>
          ...
          <Text style={{ lineHeight: 22, marginBottom: 5 }}>
            {`${weUnderstand} ${'at '}`}
          </Text>
          <Text style={{ lineHeight: 22, marginBottom: 15 }} bold>
            {`${Place} ${inDate} ${moment.utc(fromTime).format('DD.MM.YY')} ${fromHour} ${moment.utc(toTime).local().format('HH:mm')} ${toHour} ${moment.utc(fromTime).local().format('HH:mm')}`}
          </Text>
        ...
        <View style={{ alignItems: 'center', paddingHorizontal: 25 }}>
          ...
          <View style={[styles.actionButtonsWrapper, { flexDirection: isRTL ? 'row-reverse' : 'row' }, IS_SMALL_SCREEN && { marginBottom: 100 }]}>
            {renderActionButton(require('../../assets/main/isolation.png'), goIntoIsolation, allInstructions, () => Linking.openURL("https://www.mygov.in/covid-19/"))}
            {renderActionButton(require('../../assets/main/report.png'), reportIsolation, reportSite, () => Linking.openURL("https://www.mygov.in/covid-19/"))}
          </View>
        </View>
      </ContentContainer>
    </FadeInView>
